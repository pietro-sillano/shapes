...
Then, we have 5 free unknown parameters guessed at the beginning of the integration procedure:
...
\paragraph{South expansion for u}: how can we assume that $u(0)=0$ if we dont know the value $u_0$ ? we can do the expansion because you can always do an expansion also if the function is not close to zero in that point (apparently).

\paragraph{Volume ODE}: why or why not having a diff equation for volume??? it seems that also the one for area is not needed
