...
\section{Comparison with membrane simulations}
It would be interesting to compare these results with a membrane simulation. In our group two models are currently in use:
\begin{itemize}
  \item FLIPPY developed by George Dadunashvili \cite{dadunashviliFlippyUserFriendly2023}. It is a Montecarlo triangulated surface method, written in C++. Need to implement 
  \item Yuan model \cite{yuanOneparticlethickSolventfreeCoarsegrained2010} implemented in LAMMPS
\end{itemize}
...
\begin{itemize}
  ...
  \item After having the NorthPole expansion, try with very small contact angles $\phi$ and compare (shape,energy etc) with the unperturbed spherical vesicle $E=8 \pi k$
  \item insted of shooting from souh pole to the north pole, we can try to match the BCs in the middle of the path.
  \item Analytical expansion: monge gauge in spherical coordinates, see \cite{dadunashviliPhysicalPrinciplesMembrane} pag 32 or Timon pdf on spherical fluctuations.
  \item Improve stability of the numerical code, understand concepts of stable manifold in numerical integration \cite{farjounOptimalShapeJavelin2007} \cite{farjounTallestColumnDynamical2008}
  \item generalize the method to another system, look at extracting a membrane tube \cite{derenyiFormationInteractionMembrane2002}. We already have a (non stable) code for the tube in folder "tube".
\end{itemize}
