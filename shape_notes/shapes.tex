...
For the unbound segment the shape is given from the minimization of the energy functional $F$:
$$
F = E_{un} + \Sigma(A-A_{bo}) - \Delta P (V+V_{bo}) = E_{un} + \Sigma(A-A_{bo})
$$
In our simulations we dont have any control on the vesicle volume, we dont pay a cost to change the vesicle volume then we put the third term equal to zero.

$E_{un}$ is defined with the Helfrich energy equation:
$$
E_{un} = \int_{A_{un}} dA \; 2kH^2
$$

Assuming that the vesicle shape will be It is possible to parametrize (rewrite) $F$ in terms of $s,\psi(s),x(s)$.

$$
F = \int_{s_0}^{s^\star} L(s,\psi(s),x(s)) \; ds
$$

\alert{Try to rewrite $F$, mean curvature $H$ in function of $s$}
...
We want to minimize the functional $\delta F = 0$ (in the same way you can do for the action functional in classical mechanics) to find the equilibrium (minimum energy) shape. 

After the minimization you end up with a system of differential equations that once solved it will describe the vesicle shape.
