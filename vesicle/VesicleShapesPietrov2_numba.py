...
def reconstruct_and_save_plot(deg, opt_params, boundary_conditions_ds, rpa):
    ...
    # H_ds = hamiltonian(sol_full, sigma_ds)
    # plt.plot(sol_full.t, H_ds)
    # plt.title(f"Hamiltonian (Double Shooting, deg={deg})")
    # plt.savefig(f"plot/hamiltonian_deg{deg}.png")
    # plt.clf()

    filename = f"numba_rpa_{rpa:.2f}_phi_{deg:1f}.png"
...
def main():
    ...
    initial_guess = [8.76545115, 0.21125935, 0.21337616, 6.39789426]
    ...
    save_best_params(filename, result_ds.x, rpa, deg, result_ds.cost)
