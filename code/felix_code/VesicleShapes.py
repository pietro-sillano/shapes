...
def South_Gamma(s, omega, m, u0, delta_p, sigma):
    ...
    gamma3 = omega**3*(m*u0*(m-u0)*(u0-3*m)-1/4*(9*u0-3*m)
                       * delta_p+1/2*u0*(u0-3*m)*sigma)
...
def North_Gamma(epsilon, omega, m, u1, delta_p, sigma):
    ...
    gamma3 = omega**3*(m*u1*(m-u1)*(3*m-u1)+1/4*(9*u1-3*m)
                       * delta_p+1/2*u1*(3*m-u1)*sigma)
...
def ShapeJacobian(s, z, omega, u0, u1, delta_p, sigma, m, nu):
    ...
    a31 = omega*(u*np.cos(psi)/radius**2-1/2*np.cos(psi)*delta_p -
                 gamma*np.sin(psi)/radius**2-2*np.cos(psi)*np.sin(psi)/radius**3)
    a32 = omega*(gamma*np.cos(psi)/radius+np.cos(psi)**2/radius**2+u*np.sin(psi) /
                 radius+1/2*radius*delta_p*np.sin(psi)-np.sin(psi)**2/radius**2)
    a33 = -omega*np.cos(psi)/radius
    a34 = omega*np.sin(psi)/radius
    a35, a36 = 0, 0
    a41 = omega*(-delta_p*np.sin(psi)+np.sin(psi)**2/radius**3)
    a42 = omega*(-radius*np.cos(psi)*delta_p -
                 np.cos(psi)*np.sin(psi)/radius**2)
    a43 = omega*(-2*m + u)
    a44, a45, a46 = 0, 0, 0
    a51, a52, a53, a54, a55, a56 = omega/2, 0, 0, 0, 0, 0
    a61, a62, a63, a64, a65, a66 = 3/2*omega*radius * \
        np.sin(psi), 3/4*omega*radius**2*np.cos(psi), 0, 0, 0, 0
...
def Residuales(paras, shape_parameters):
    ...
    sol = solve_ivp(ShapeIntegrator, [s_init, 1-s_init], z_init,
                    jac=ShapeJacobian, args=parameters, t_eval=s, method='Radau')
...
def Residuales_scalar(paras, shape_parameters):
    omega, u0, u1, delta_p, sigma = paras
    m, nu = shape_parameters
    ...
    return r1**2 + r2**2 + r3**2 + r4**2 + r5**2 + r6**2
...
def PlotShapes(result, shape_parameters, gamma_list, nu_list):
    ...
    sub1.plot(radius, z, color="blue", linestyle="--", linewidth=7,
              label=r'$\nu$: "+str(nu)+", $\bar{H}_0$: "+str(m)+', alpha=0.5)
    sub1.plot(-radius, z, color="red", linestyle="--",
              linewidth=7, label="numeric", alpha=0.5)
    sub1.axis('off')
    sub1.set_aspect("equal")
    plt.savefig("Red_Vol="+str(nu)[:]+"Red_Pref_Curv=" +
                str(m)[:]+".pdf", bbox_inches='tight', pad_inches=0)
...
def main():
    ...
    for index in range(len(para_dict)):
        ...
        result = least_squares(Residuales, parameters_init, args=(
            [shape_parameters]), method='lm')
